    ...
    <div className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <main className="mx-auto max-w-4xl px-4 py-6">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {STARTERS.map((s) => (
                <button
                  ...
                  className="rounded-xl border-2 border-slate-200 bg-white p-4 text-left transition hover:border-emerald-500 hover:shadow-md"
                >
                  <div className="text-lg font-semibold">{s.name}</div>
                  <div className="text-slate-600">{s.role}</div>
                  <div className="mt-1 text-sm text-emerald-700">{s.mode}</div>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-4">
          {messages.map((m, i) => (
            <div key={i}>
              <div
                className={
                  m.role === "user"
                    ? "ml-auto max-w-[85%] rounded-2xl bg-emerald-700 px-4 py-3 text-lg text-white"
          ...
          {loading && (
            <div className="max-w-[95%] rounded-2xl border border-slate-200 bg-white px-4 py-3 text-lg text-slate-500">
              <span className="inline-block animate-pulse">Looking that up…</span>
            </div>
          )}
        ...
        <div className="mx-auto max-w-4xl">
          <div className="flex items-end gap-2">
            ...
            <textarea
              ...
              rows={1}
              className="min-h-14 flex-1 resize-none rounded-xl border-2 border-slate-300 px-4 py-3 text-lg focus:border-emerald-600 focus:outline-none"
            />

            <button
              onClick={() => void send(input)}
              disabled={loading || !input.trim()}
              className="h-14 shrink-0 rounded-xl bg-emerald-700 px-6 text-lg font-medium text-white disabled:opacity-40"
            >
              Send
            </button>
          </div>
